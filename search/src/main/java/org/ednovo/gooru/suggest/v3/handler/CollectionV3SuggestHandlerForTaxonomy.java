...
@Component
public class CollectionV3SuggestHandlerForTaxonomy extends SuggestHandler<Map<String, Object>> implements Constants {

	protected static final Logger LOG = LoggerFactory.getLogger(CollectionV3SuggestHandlerForTaxonomy.class);
	...
	@Autowired
	private SCollectionDeserializeProcessor collectionDeserializeProcessor;

	@Autowired
	private CollectionSuggestDeserializeProcessor collectionSuggestDeserializeProcessor;
	...
	protected SuggestHandlerType getType() {
		return SuggestHandlerType.TAXONOMY_COLLECTION;
	}
	...
	public SuggestResponse<Object> suggest(final SuggestData suggestData, Map<SuggestDataProviderType, Object> dataProviderInput) {
		...
		suggestData.setType(TYPE_SCOLLECTION);
		...
		try {
			...
			if (taxonomyData != null) {
				...
				if (ids != null && !ids.isEmpty()) {
					...
					LOG.info("Checking other published collections as quality suggestions are unavailable for concept nodes : " + taxonomyData.getSlInternalCodes());
			...
			if (suggestData.getIsIntenralSuggest()) {
				collectionSuggestDeserializeProcessor.process(suggestData, suggestResponseResource);
				suggestResponse.setSuggestResults(suggestResponseResource.getSearchResults());
			} else {
				collectionDeserializeProcessor.process(suggestData, searchResponseResource);
